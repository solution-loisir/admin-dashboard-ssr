...
const auth = require("../../../src/_data-helpers/auth");
const getActivities = require("../../../src/_data-helpers/get-activities");
const getSpotsStats = require("../../../src/_data-helpers/get-spots-stats");
...
async function handler(event) {
  ...
  if (event.queryStringParameters.id) {
    token = await auth();
    activities = await getActivities(token, event.queryStringParameters.id);
  }
  ...
  let elev = new EleventyServerless("dynamic", {
    ...
    config(config) {
      if (this.query.id) {
        config.addGlobalData("activities", async () => {
          return activities;
        });
        config.addGlobalData("spotsStats", () => getSpotsStats(activities));
      }
    }
  });
